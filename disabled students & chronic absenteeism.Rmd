...
worst_districts <- worst_ratios %>% group_by(district_code) %>% summarize(agency_code = n()) %>% arrange(desc(agency_code))

#33 of these schools are concentrated in 10 districts. About 28.7 percent 


#Now using the district_code column, let's find out if those schools are heavily concentrated in a particular county or counties. 
best_districts <- best_ratios %>% group_by(district_code) %>% summarize(agency_code = n()) %>% arrange(desc(agency_code)). 
